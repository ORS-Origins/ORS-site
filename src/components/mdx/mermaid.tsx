...
export function Mermaid({ chart }: { chart: string }) {
  ...
  const preMaximizeScaleRef = useRef<number>(mermaidConfig.defaultScale);
  ...
  const labels =
    MERMAID_LABELS[(locale ?? i18n.defaultLanguage) as keyof typeof MERMAID_LABELS] ??
    MERMAID_LABELS[i18n.defaultLanguage];
  ...
  useLayoutEffect(() => {
    ...
    const BBOX_PADDING = mermaidConfig.bboxPadding;
    let minX = vb.x;
    let minY = vb.y;
  ...
  const recomputeFitCanvasScale = useCallback(() => {
    ...
    if (svgW <= 0 || svgH <= 0) {
      setFitCanvasScale(mermaidConfig.maxScale);
      return;
    }
    ...
    setFitCanvasScale(Math.min(mermaidConfig.maxScale, availW / svgW, availH / svgH));
  ...
  const zoomIn = useCallback(() => {
    setScale((s) => Math.min(mermaidConfig.maxScale, +(s + mermaidConfig.scaleStep).toFixed(2)));
  }, []);

  const zoomOut = useCallback(() => {
    setScale((s) => Math.max(mermaidConfig.minScale, +(s - mermaidConfig.scaleStep).toFixed(2)));
  }, []);

  const resetZoom = useCallback(() => {
    setScale(mermaidConfig.defaultScale);
    setPan(ORIGIN);
  }, []);
  ...
  const computeFitViewportScale = useCallback((): number => {
    const svgW = svgNatural.width;
    const svgH = svgNatural.height;
    if (svgW <= 0 || svgH <= 0) return mermaidConfig.defaultScale;
    if (typeof window === 'undefined') return mermaidConfig.defaultScale;
    const availW = window.innerWidth - mermaidConfig.viewportFitPadding;
    const availH =
      window.innerHeight - mermaidConfig.viewportFitPadding - mermaidConfig.viewportToolbarGap;
    if (availW <= 0 || availH <= 0) return mermaidConfig.defaultScale;
    return Math.min(availW / svgW, availH / svgH, mermaidConfig.maxScale);
  }, [svgNatural.width, svgNatural.height]);
  ...
  const canZoomOut = scale > mermaidConfig.minScale;
  const canZoomIn = scale < mermaidConfig.maxScale;
  ...
  // `mermaidConfig.defaultScale`.
  // 缩放或平移任一偏离 DEFAULT_SCALE 即可重置；用极小 epsilon 兜底
  // 浮点精度抖动。
  const canReset =
    Math.abs(scale - mermaidConfig.defaultScale) > mermaidConfig.resetEpsilon ||
    pan.x !== 0 ||
    pan.y !== 0;
